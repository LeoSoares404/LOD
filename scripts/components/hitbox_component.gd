...
@export var slow_stacks := false   # true = soma no slow atual (lentidão da zarabatana)
...
## > 0 = também aplica o debuff de "pegando fogo" no alvo (dano/tick por esse valor).
## Usado no inferno: aura dos inimigos melee e projéteis dos ranged.
